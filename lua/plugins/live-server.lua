...
local function start_server(project_root)
  ...
  local vite_cmd = string.format(
    "%s dev --port %s --force '%s'",
    vite_bin,
    project_port,
    project_root
  )
  ...
  local job_id = vim.fn.jobstart({ "zsh", "-i", "-c", vite_cmd }, {
    on_stdout = function(_, data)
      ...
      if output:find("Local:") or output:find("http://localhost:" .. project_port) then
        vim.schedule(function()
          ...
          vim.notify("Vite [" .. vim.fn.fnamemodify(project_root, ":t") .. "] ready at " .. full_url, vim.log.levels.INFO)
    ...
    on_stderr = function(_, data)
      ...
      if err ~= "" 
         ...
         and not err:find("Thefollowingdependenciesareimported") then
        vim.schedule(function()
  ...
  vim.notify("Starting Vite on port " .. project_port .. " for " .. vim.fn.fnamemodify(project_root, ":t") .. "...", vim.log.levels.INFO)
end
